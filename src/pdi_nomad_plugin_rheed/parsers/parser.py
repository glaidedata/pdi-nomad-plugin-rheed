    ...
    def _populate_schema_from_row(self, result_obj, row):
        ...
        for csv_col, schema_var in CSV_TO_SCHEMA_MAP.items():
            if csv_col in row and pd.notna(row[csv_col]):
                val = row[csv_col]
                if hasattr(sample, schema_var): setattr(sample, schema_var, val)
                elif hasattr(settings, schema_var): setattr(settings, schema_var, val)
                elif hasattr(result_obj, schema_var): setattr(result_obj, schema_var, val)
