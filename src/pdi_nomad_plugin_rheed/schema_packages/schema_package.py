...
class ChamberGeometry(ArchiveSection):
    distance_sample_to_screen_mm = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
...
class Camera(ArchiveSection):
    ...
    image_length_calibration_mm_per_px = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))

class InstrumentSettings(ArchiveSection):
    electronics_type = Quantity(type=MEnum('FUG', 'STAIB'), a_eln=dict(component='EnumEditQuantity'))
...
class EGunSTAIB(ArchiveSection):
    electron_energy_keV = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    emission_current_uA = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    filament_current_A = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))

class EGunFUG(ArchiveSection):
    electron_energy_keV = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    emission_current_uA = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    filament_current_A = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    filament_voltage_V = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
...
class DeflectionUnitFUG(ArchiveSection):
    ...
    x_or_y_scan = Quantity(type=MEnum('x', 'y'), a_eln=dict(component='EnumEditQuantity'))
    ext_or_int_ref = Quantity(type=MEnum('ext', 'int'), a_eln=dict(component='EnumEditQuantity'))
    beam_deflection_x_coarse = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    beam_deflection_x_fine = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    beam_deflection_x_crosspoint = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    beam_deflection_x_angle = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    beam_deflection_y_coarse = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    beam_deflection_y_fine = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    beam_deflection_y_crosspoint = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
    beam_deflection_y_angle = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
...
class RHEEDMeasurementSettings(ArchiveSection):
    ...
    incidence_angle_deg = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
...
class SubstrateHolder(ArchiveSection):
    rotation_angle_alpha_deg = Quantity(type=float, a_eln=dict(component='NumberEditQuantity'))
...
class Sample(ArchiveSection):
    ...
    sample_surface_compound = Quantity(type=str, a_eln=dict(component='StringEditQuantity'))
...
class RHEEDImageResult(RHEEDResult):
    ...
    derived_from_video_link = Quantity(type=str, a_eln=dict(component='StringEditQuantity'))
...
class PointScan(ArchiveSection):
    ...
    sensor_position_overview_picture = Quantity(type=str, a_browser=dict(adaptor='RawFileAdaptor'))
    sensor_definition_file = Quantity(type=str, a_browser=dict(adaptor='RawFileAdaptor'))
